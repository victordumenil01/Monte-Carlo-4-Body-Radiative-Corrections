...
#Gardner
def I(dE, E2, delta):
    beta = np.sqrt( 1 - 1/pow(E2,2))
    ligne1 = -4*np.log((delta-E2)/dE) - 4*(delta-E2-dE)/E2 + 8*(delta-E2-dE)/(delta-E2)
    ligne2 = 4*(pow(delta-E2,2)-pow(dE,2))/(E2*delta-pow(E2,2)) - 2*(pow(delta-E2,2)-pow(dE,2))/pow(delta-E2, 2) - (4/3)*(pow(delta-E2,3)-pow(dE,3))/(E2*pow(delta-E2, 2))
    ligne3 = 2*np.log((delta-E2)/dE) + 2*(delta-E2-dE)/E2 + (pow(delta-E2,2)-pow(dE,2))/(2*pow(E2,2))
    ligne4 = -4*(delta-E2-dE)/(delta-E2) - 2*(pow(delta-E2,2)-pow(dE,2))/(E2*delta-pow(E2,2)) - (2/3)*(pow(delta-E2,3)-pow(dE,3))/(pow(E2,2)*(delta-E2))
    ligne5 = (pow(delta-E2,2)-pow(dE,2))/pow(delta-E2, 2) + (2/3)*(pow(delta-E2,3)-pow(dE,3))/(E2*pow(delta-E2, 2)) + (1/4)*(pow(delta-E2,4)-pow(dE,4))/(pow(E2,2)*pow(delta-E2, 2))
    return ligne1 + ligne2 + (2/beta) * np.arctanh(beta) * (ligne3 + ligne4 + ligne5)
    
def Ik_1(k, kmax, eps):
    betae = np.sqrt( 1 - 1/pow(eps,2))
    betak = np.sqrt( 1 - 1/pow(eps-k,2))
    return np.log( ((1+betak)*(1-betae))/((1+betae)*(1-betak)) )/k
    
def Ik_n(k, kmax, eps, n):
    betak = np.sqrt( 1 - 1/pow(eps-k,2))
    return pow(k, n)*np.log( (1+betak)/(1-betak) )
 
def int_Ik0(kmax, eps, n):
    i = integrate.quad(Ik_n, 0, kmax, args=(kmax, eps, n))
    return i[0]

def int_Ik1(kmax, eps, n):
    i = integrate.quad(Ik_n, 0, kmax, args=(kmax, eps, n))
    return i[0]

def int_Ik_1(kmax, eps):
    i = integrate.quad(Ik_1, 0, kmax, args=(kmax, eps))
    return i[0]
      
    
def I_l(kmax, eps, l):
    betae = np.sqrt( 1 - 1/pow(eps,2))
    ligne1 = 6 - 4*(1 - np.arctanh(betae)/betae)*np.log(2*kmax/l) + 2*np.arctanh(betae)/betae + (2/betae)*(-spence(1-(2 * betae/(1 + betae))))
    ligne2 = - 2*pow(np.arctanh(betae),2)/betae + (2/betae)*int_Ik_1(kmax, eps) - 2*int_Ik0(kmax, eps, 0)/(eps*betae) + int_Ik1(kmax, eps,1)/(pow(eps, 2)*betae) - 4*np.sqrt( pow(eps-kmax, 2)-1)/np.sqrt( pow(eps,2)-1)
    ligne3 = (4/betae)*np.log( (eps-kmax-np.sqrt( pow(eps-kmax, 2)-1))/(eps-np.sqrt( pow(eps,2)-1)) )
    ligne4 = 4*np.log( (pow(eps, 2) - 1 - eps*kmax + np.sqrt(pow(eps,2)-1)*np.sqrt(pow(eps-kmax,2)-1))/(2*(pow(eps,2)-1)) )
    
    return ligne1 + ligne2 + ligne3 + ligne4

def gb(dE, E2, l, delta):
    return np.heaviside( -(dE+1-E2), 0)*I_l(E2-1, E2, l) + np.heaviside( -(E2-(dE+1)), 0)*I_l(dE, E2, l) + np.heaviside( -(delta-(dE-E2)), 0)*I(dE, E2, delta)
 
def gvv(E2, l):
    beta = np.sqrt( 1 - 1/pow(E2,2))
    A = 0.5*beta*np.log( (1+beta)/(1-beta) ) - 1 + 2*np.log(l)*( (0.5/beta)*np.log( (1+beta)/(1-beta) ) - 1) + 1.5*np.log(mp/me) - (1/beta)*pow( 0.5*np.log( (1+beta)/(1-beta) ),2) + (1/beta)*(-spence(1 - (2*beta/(1+beta))))
    return 2*A - 3/4    

def g(dE, E2, l, delta):
    return 1 + (ALPHA/(2*np.pi))*(gb(dE, E2, l, delta) + gvv(E2, l)) -0.008
...
def g_gardner(dE, E2, l, delta):
    ...
    delta : Q-value (keV)
    """
    return 1.0 + (ALPHA / (2.0 * np.pi)) * (_gb(dE, E2, l, delta) + _gvv(E2, l)) - 0.008